...
onready var items_cat = [
	{
		ITEM_REFRESH: Lang.use("item_refresh")
	},

	{
		ITEM_CHECKOUT_BRANCH: Lang.use("item_checkout_branch"),
		ITEM_DELETE_BRANCH: Lang.use("item_delete_branch")
	},

	{
		ITEM_COMMIT_ALL: Lang.use("item_commit_all"),
		ITEM_REVERT_ALL: Lang.use("item_revert_all"),
	},

	{
		ITEM_CREATE_BRANCH: Lang.use("item_create_branch"),
		ITEM_MERGE_BRANCH: Lang.use("item_merge_branch"),
		ITEM_REBASE_BRANCH: Lang.use("item_rebase_branch")
	},

	{
		ITEM_TAG: Lang.use("item_tag")
	},

	{
		ITEM_GITIGNORE_MANAGER: Lang.use("item_gititnore_manager"),
		ITEM_SETTINGS: Lang.use("item_settings")
	},

	{
		ITEM_LAUNCH_TERMINAL: Lang.use("node_text_launch_terminal_btn")
	}
]
